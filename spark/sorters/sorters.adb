...
package body Sorters is
   ...
   function Perm_Transition (A, B, C : Array_Type) return Boolean
     with Global => null,
     Post => (if Perm_Transition'Result
              and then Perm (A, B)
              and then Perm (B, C)
                  then Perm (A, C)),
     Ghost => True,
   ...
   begin -- Selection_Sort
      ...
      for Current in Values'First .. Values'Last - 1 loop
         ...
         pragma Assume (Perm_Transition (Values'Loop_Entry, Values_Last, Values));
